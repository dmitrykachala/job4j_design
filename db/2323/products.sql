...
select t.name as type, p.name, p.price from product as p join type as t on t.name = 'сыр' and p.type_id = t.id;
...
select t.name, count(p.type_id) as count from product as p join type as t on p.type_id = t.id group by t.name;

select t.id, p.name, p.price from product as p join type as t on p.type_id = t.id and (t.name ='сыр' 
or t.name ='молоко') order by t.id;

select t.id, t.name, count(p.type_id) as count from product as p join type as t on p.type_id = t.id 
group by t.id having count(p.type_id) < 10 order by t.id;

select t.name as type, p.name, p.price from product as p join type as t on p.type_id = t.id 
group by t.name, p.name, p.price order by t.name;